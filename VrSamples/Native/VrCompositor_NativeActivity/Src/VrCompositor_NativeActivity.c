...
// If 'numberOfMipmapLevels' is -1 and 'data' != NULL, then the full mip chain will be generated from the finest data level.
static ovrTextureSwapChain * ovrTextureSwapChain_Create( const char * fileName,
										const int width, const int height, const int depth,
										const int numberOfArrayElements, const int numberOfFaces, const int numberOfMipmapLevels,
										const GLenum glInternalFormat, const void * data, const size_t dataSize, const bool mipSizeStored )
{
	...
	if ( numberOfFaces <= 1 )
	{
		chain = vrapi_CreateTextureSwapChain( VRAPI_TEXTURE_TYPE_2D, VRAPI_TEXTURE_FORMAT_8888, width, height, numStorageLevels, false );
	}
	else
	{
		chain = vrapi_CreateTextureSwapChain( VRAPI_TEXTURE_TYPE_CUBE, VRAPI_TEXTURE_FORMAT_8888, width, height, numStorageLevels, false );
	}
	...
	if ( data != NULL )
	{
		...
		for ( int i = 0; i < numDataLevels; i++ )
		{
			...
			switch ( glInternalFormat )
			{
				...
				case GL_RGB8:	{ mipSize = w * h * d * 3 * sizeof( unsigned char ); glFormat = GL_RGB;  glDataType = GL_UNSIGNED_BYTE; break; }
			...
			if ( depth <= 1 && numberOfArrayElements <= 1 )
			{
				for ( int face = 0; face < numberOfFaces; face++ )
				{
					...
					{
						ALOGE( "%s: Mip %d data exceeds buffer size (%zu > %zu)", fileName, i, mipSize, ( endOfBuffer - levelData ) );
						GL( glBindTexture( glTarget, 0 ) );
						return chain;
					}
...
#pragma pack()
	...
	ovrTextureSwapChain * chain = ovrTextureSwapChain_Create( fileName,
									header->pixelWidth, header->pixelHeight, header->pixelDepth,
									numberOfArrayElements, numberOfFaces, numberOfMipmapLevels,
									header->glInternalFormat, buffer + startTex, bufferSize - startTex, true );
...
static void ovrScene_Clear( ovrScene * scene )
{
	...
	memset( &scene->StaticImageFence, 0, sizeof( scene->StaticImageFence ) );
...
static void ovrScene_Create( ovrScene * scene )
{
	...
	{
		scene->CubeMapSwapChain = ovrTextureSwapChain_CreateFromKTX( "/sdcard/oculus/cubemap1536.ktx" );
	}
	...
	// Simple checkerboard pattern.
	{
		...
		scene->EquirectSwapChain = vrapi_CreateTextureSwapChain( VRAPI_TEXTURE_TYPE_2D,
								VRAPI_TEXTURE_FORMAT_8888, width, height, 1, false );
	...
	// Simple checkerboard pattern.
	{
		...
		scene->CylinderSwapChain = vrapi_CreateTextureSwapChain( VRAPI_TEXTURE_TYPE_2D,
								VRAPI_TEXTURE_FORMAT_8888, scene->CylinderWidth, scene->CylinderHeight, 1, false );

		uint32_t * texData = (uint32_t *)malloc( scene->CylinderWidth * scene->CylinderHeight * sizeof( uint32_t ) );
		...
		{
			for ( int i = 0; i < scene->CylinderWidth; i++ )
			{
				texData[i] = 0x00000000;
				texData[ ( ( scene->CylinderHeight - 1 ) * scene->CylinderWidth + i )] = 0x00000000;
			}
			for ( int i = 0; i < scene->CylinderHeight; i++ )
			{
				texData[ i * scene->CylinderWidth] = 0x00000000;
				texData[ ( i * scene->CylinderWidth + scene->CylinderWidth - 1 )] = 0x00000000;
			}
		}

		const int texId = vrapi_GetTextureSwapChainHandle( scene->CylinderSwapChain, 0 );
		glBindTexture( GL_TEXTURE_2D, texId );
		glTexSubImage2D( GL_TEXTURE_2D, 0, 0, 0, scene->CylinderWidth, scene->CylinderHeight, GL_RGBA, GL_UNSIGNED_BYTE, texData );
	...
	// Insert a fence to guard static image generation.
	{
		ovrFence_Create( &scene->StaticImageFence );
		ovrFence_Insert( &scene->StaticImageFence );
	}
...
static void ovrScene_Destroy( ovrScene * scene )
{
	...
	ovrFence_Destroy( &scene->StaticImageFence );
...
static ovrLayerCube2 BuildCubeLayer( ovrTextureSwapChain * cubeSwapChain, const ovrTracking2 * tracking )
{
...
static ovrLayerEquirect2 BuildEquirectLayer( ovrTextureSwapChain * equirectSwapChain, const ovrTracking2 * tracking )
{
...
static ovrLayerCylinder2 BuildCylinderLayer( ovrTextureSwapChain * cylinderSwapChain,
	...
	if ( app->Resumed != false && app->NativeWindow != NULL )
	{
		if ( app->Ovr == NULL )
		{
			...
			if ( app->Ovr != NULL )
			{
				...
				vrapi_SetTrackingTransform( app->Ovr, vrapi_GetTrackingTransform( app->Ovr, VRAPI_TRACKING_TRANSFORM_SYSTEM_CENTER_EYE_LEVEL ) );

				ALOGV( "		vrapi_SetTrackingTransform( EYE_LEVEL )" );
			}
		}
	}
	else
...
{
	if ( app->BackButtonState == BACK_BUTTON_STATE_PENDING_SHORT_PRESS && !app->BackButtonDown )
	{
		if ( ( GetTimeInSeconds() - app->BackButtonDownStartTime ) >
...
static int ovrApp_HandleKeyEvent( ovrApp * app, const int keyCode, const int action )
{
	...
	if ( keyCode == AKEYCODE_BACK )
	{
		if ( action == AKEY_EVENT_ACTION_DOWN )
		{
			...
			{
				app->BackButtonDownStartTime = GetTimeInSeconds();
			}
		...
		if ( shouldRenderWorldLayer )
		{
			appState.Layers[appState.LayerCount++].Projection =
					ovrRenderer_RenderSceneToEyeBuffer( &appState.Renderer, &appState.Java,
						&appState.Scene, &tracking, &completionFence );
		}
		else
		{
			completionFence = (size_t)appState.Scene.StaticImageFence.Sync;
		}

		// Add a simple cylindrical layer
		appState.Layers[appState.LayerCount++].Cylinder =
			BuildCylinderLayer( appState.Scene.CylinderSwapChain,
					appState.Scene.CylinderWidth, appState.Scene.CylinderHeight, &tracking );

		// Compose the layers for this frame.
		const ovrLayerHeader2 * layerHeaders[ovrMaxLayerCount] = {};
